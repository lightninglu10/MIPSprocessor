...
void execute_one_inst(processor_t* p, int prompt, int print_regs)
{
  ...
  case 0xa: //opcode == 0xa (slti)
      if ((int32_t) p->R[inst.itype.rs] < ((int32_t) ((int16_t)(inst.itype.imm)))) {
	  p->R[inst.itype.rt] = 1;
      } else {
	  p->R[inst.itype.rt] = 0;
      }
  ...
  case 0xb: //opcode == 0xb (sltiu)
      if (p->R[inst.itype.rs] < ((uint32_t) ((uint16_t)(inst.itype.imm)))) {
	  p->R[inst.itype.rt] = 1;
      } else {
	  p->R[inst.itype.rt] = 0;
      }
